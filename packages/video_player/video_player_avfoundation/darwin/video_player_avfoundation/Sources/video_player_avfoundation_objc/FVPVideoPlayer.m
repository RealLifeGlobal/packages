...
#import "./include/video_player_avfoundation_objc/FVPBackgroundAudioHandler.h"
#import "./include/video_player_avfoundation_objc/FVPPipController.h"
...
- (void)enterPip {
  if (!_pipController) {
    _pipController = [[FVPPipController alloc] initWithPlayerLayer:self.playerLayer];
...
- (BOOL)isPipActive {
  return _pipController.isPipActive;
}
...
- (void)setAutoPipEnabled:(BOOL)enabled {
  ...
  [_pipController setCanStartAutomatically:enabled];
}
...
- (void)enableBackgroundPlaybackWithTitle:(nullable NSString *)title
  ...
  if (!_backgroundAudioHandler) {
    _backgroundAudioHandler = [[FVPBackgroundAudioHandler alloc] initWithPlayer:_player];
  }
  [_backgroundAudioHandler enableWithTitle:title
                                    artist:artist
